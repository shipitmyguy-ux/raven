...
export function validateDraft(kind,draft,profile,{target=null,instructions=""}={}){
  ...
  return {name:profile.name,contact:profile.contact,headline:groundedText(draft.headline,profile,{max:160,target,instructions}),
    summary:groundedText(draft.summary,profile,{max:1600,target,instructions}),skills,experience,
...
const writingInstructions=[
  ...
  "For a resume: use implied first person without I/my. Write a short headline, a two-sentence summary and purposeful bullets. Usually 8-12 carefully chosen skills are enough; do not dump the skill catalog. For Games / 3D roles, include every work-history entry whose id appears in resume_required_experience_ids and never include SoundAir unless the revision request explicitly asks for SoundAir by name. For Professional, Labor, and Wildcard roles, do NOT force the full game-art chronology: select only the work-history entries that materially support the target role, keep art-production details concise, and foreground transferable evidence such as team leadership, mentoring, onboarding/training, project delivery, internal meeting leadership, Excel, automation scripting/module building, asset-database metadata/reporting/querying, cross-functional coordination, and hands-on maintenance when relevant. It is acceptable for non-game resumes to omit old or irrelevant game-art roles. Use additional only for useful career highlights not already covered. Preserve exact skill names and use experience_id to refer to work history. Raven will restore identity, dates, employer names, titles and education unchanged.",
