...
export async function upsertResults(rows:Candidate[]){
  ...
  const body=rows.map(c=>({
    track:c.track,title:c.title,company:c.company||null,location:c.location||null,
    remote:Boolean(c.remote),salary_text:c.salary_text||null,url:c.url,source:c.source,
    snippet:c.snippet||null,score:c.score||0,status:"Discovered",last_seen:now
  }));
