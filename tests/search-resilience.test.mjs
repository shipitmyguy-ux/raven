...
assert.match(search,/linkedinDeep\(track,6\)/,"deep LinkedIn work must stay bounded");
assert.match(search,/jobicy\(track,4\)/,"deep Jobicy work must stay bounded");
assert.match(search,/himalayas\(track,4\)/,"deep Himalayas work must stay bounded");
assert.ok(!/const enriched=await enrichRows\(rows\)/.test(search),"deep search must not block on per-job page enrichment");

assert.match(sources,/maxLines=quick \? 2200 : 7000/,"ATS CSV scan lines must be bounded");
assert.match(sources,/maxBytes=quick \? 2\*1024\*1024 : 7\*1024\*1024/,"ATS CSV bytes must be bounded");
